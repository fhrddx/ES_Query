            ...
            for (int i = 0; i < 1000000; i++)
            {
                ...
                try
                {
                    Student model = new Student()
                    {
                        ...
                        @class = i,
                        desc = "华南理工大学" //TestData.content.Substring((r.Next(0, content_length - 700)), 100)
                    };
                    await ElasticSearchHelper.Intance.CreateIndex("db_student_test1", i.ToString(), model);
                    model = null;
                }
                catch (Exception ex)
                {
